	...
	runCases := []testCase{
		{
			...
			expected: [][]byte{
				...
				[]byte(
					`{"name":"Quest for the Holy Grail","description":"Tasks related to finding the Grail","taskCount":5}`,
				),
			},
		},
		{
			inputs: []any{
				...
				JelloBoard{
					...
					"Prepare for battle with the Rabbit of Caerbannog",
					7,
				},
				JelloBoard{
					"Use the Holy Hand Grenade",
					"Instructions on deploying the Holy Hand Grenade of Antioch",
					3,
				},
			},
			expected: [][]byte{
				...
				[]byte(
					`{"name":"Defeat the Killer Rabbit","description":"Prepare for battle with the Rabbit of Caerbannog","taskCount":7}`,
				),
				[]byte(
					`{"name":"Use the Holy Hand Grenade","description":"Instructions on deploying the Holy Hand Grenade of Antioch","taskCount":3}`,
				),
			},
		},
	}
	...
	submitCases := append(runCases, []testCase{
		{
			...
			expected: [][]byte{
				[]byte(
					`{"id":"004","name":"Sir Robin the Not-Quite-So-Brave-As-Sir-Lancelot","role":"Minstrel"}`,
				),
				[]byte(
					`{"name":"Avoid Dangerous Situations","description":"Strategies for running away bravely","taskCount":2}`,
				),
			},
		},
	}...)
